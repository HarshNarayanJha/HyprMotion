import { animationNames, type Animation, type AnimationName, type Bezier, type Style } from "$lib/types";
...
export class HyprAnimationParser {
  ...
  private parseBezier(line: string): void {
    ...
    if (parts.length !== 6) {
      throw new Error("Invalid bezier format. Expected: bezier = NAME, X0, Y0, X1, Y1");
    }
  ...
  private parseAnimation(line: string): void {
    const parts = this.splitConfigLine(line);
    if (parts.length < 5) {
      throw new Error("Invalid animation format. Expected: animation = NAME, ONOFF, SPEED, CURVE [,STYLE]");
    }
    ...
    if (!onoff) {
      this.animations.set(name, { name, onoff } as Animation);
      return;
    }
        ...
        switch (baseStyle) {
          case "slide":
            if (!["left", "right", "top", "bottom"].includes(param)) {
              throw new Error(`Invalid slide direction: ${param}. Must be left, right, top, or bottom`);
            }
          ...
          case "popin": {
            ...
            if (!popinMatch) {
              throw new Error(`Invalid popin percentage: ${param}. Must be in format: NUMBER%`);
            }
            const popinPercent = Number.parseInt(popinMatch[1], 10);
            if (popinPercent < 0 || popinPercent > 100) {
              throw new Error(`Invalid popin percentage: ${popinPercent}. Must be between 0 and 100`);
            }
          ...
          case "slidefadevert": {
            ...
            if (!fadeMatch) {
              throw new Error(`Invalid slide fade percentage: ${param}. Must be in format: NUMBER%`);
            }
            const fadePercent = Number.parseInt(fadeMatch[1], 10);
            if (fadePercent < 0 || fadePercent > 100) {
              throw new Error(`Invalid slide fade percentage: ${fadePercent}. Must be between 0 and 100`);
            }
  ...
  private isValidStyle(animation: AnimationName, style: string): boolean {
    ...
    const workspaceStyles = new Set(["slide", "slidevert", "fade", "slidefade", "slidefadevert"]);
    ...
    // Windows animations
    if (["windows", "windowsIn", "windowsOut", "windowsMove"].includes(animation)) {
